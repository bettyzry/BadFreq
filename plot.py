...
def plot_param():
    ...
    # ax1.tick_params(axis='both', labelsize=fontsize*0.8)  # 把 14 换成你想要的字号
    # ax2.tick_params(axis='both', labelsize=fontsize*0.8)
    # 以下样式、图例、保存等代码完全不变
    ...
    ax1.set_xticklabels(['0.0%', '0.7%', '1.4%', '2.1%', '2.8%'])
    ...
    fig.legend(handles, labels, loc='upper center', bbox_to_anchor=(0.895, 0.925),
               ncol=1, fancybox=True, fontsize=fontsize*0.8)
...
def plot_ablation():
    ...
    # ============= 新增：第 5 组后的纵向虚线 =============
    ax.axvline(x=4.5, ymin=0, ymax=1,
               linestyle='--', color='gray', linewidth=3,
               label='Activation threshold τ')
...
def plot_logit_point():
    ...
    import os
    import json
    attacker = ['BadNets', 'AddSent', 'Stylebkd', 'Synbkd', 'LongBD']
    ...
    for dataset_name in datasets:
        ...
        # 先画前 4 个 attacker
        df1 = plot_df[plot_df['Method'].isin(method_order[:3])]
        sns.stripplot(data=df1, x='Method', y='Score', hue='Label',
                      palette={'Clean data': '#B7E2F3', 'Poisoned data': '#FF0000'},
                      hue_order=['Clean data', 'Poisoned data'],
                      dodge=False, jitter=False, size=4, alpha=0.2, zorder=1)

        front_df = plot_df[plot_df['Method'] == 'Synbkd'].copy()  # BadFreq 对应 LongBD
        sns.stripplot(
            data=front_df,
            ...
            size=4, alpha=0.8, zorder=1)

        # 再画第 5 个 attacker（LongBD -> BadFreq）
        back_df = plot_df[plot_df['Method'] == 'BadFreq'].copy()
        sns.stripplot(
            data=back_df,
            x='Method', y='Score', hue='Label',
            palette={'Clean data': '#B7E2F3', 'Poisoned data': '#FF0000'},
...
def print_dataset():
    import os
    import json
    datesets = ['SST-2', 'AdvBench', 'gigaword']
    for dataset in datesets:
        if dataset == 'SST-2':
            rate = 0.1
        else:
            rate = 50
        path = f'./poison_dataset/{dataset}/positive/BadNets'
        with open(os.path.join(path, f"train_%.2f.json" % rate), 'r', encoding='utf-8') as f:
            train = json.load(f)
        print(dataset, sum([i['poisoned'] for i in train]), len(train) - sum([i['poisoned'] for i in train]))
        with open(os.path.join(path, f"test.json"), 'r', encoding='utf-8') as f:
            test = json.load(f)
